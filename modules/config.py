...
path_flux_models = get_dir_or_set_default('path_flux', '../models/flux/')
...
def get_flux_model_names():
    if not os.path.isdir(path_flux_models):
        return []
    
    flux_models = []
    # Find directories (diffusers format)
    flux_models.extend([d for d in os.listdir(path_flux_models) if os.path.isdir(os.path.join(path_flux_models, d))])
    # Find single files
    flux_models.extend(get_files_from_folder(path_flux_models, ['.safetensors', '.gguf']))
    
    return list(set(flux_models)) # Use set to remove duplicates


def update_all_model_names():
    global model_filenames, lora_filenames, flux_model_filenames
    model_filenames = get_model_filenames(path_checkpoints)
    lora_filenames = get_model_filenames(path_loras)
    flux_model_filenames = get_flux_model_names()
    model_filenames += flux_model_filenames
    return
